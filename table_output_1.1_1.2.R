...
for (i in 1:nrow(param_grid)) {
  ...
  file_cmte  <- sprintf("results_1.1_1.2/coef_est_cmte_n%d_p%d_r%s_eps%.2f_fn%d_rep%d.RData",
                        n, p, r_str, eps, f_num, n_rep)
  file_trr   <- sprintf("results_1.1_1.2/coef_est_trr_n%d_p%d_r%s_eps%.2f_fn%d_rep%d.RData",
                        n, p, r_str, eps, f_num, n_rep)
  file_tmddm <- sprintf("results_1.1_1.2/coef_est_tmddm_n%d_p%d_r%s_eps%.2f_fn%d_rep%d.RData",
                        n, p, r_str, eps, f_num, n_rep)
...
cat("\\end{tabular}\n")
cat("\\caption{Estimation Accuracy and Runtime for \\(\\beta_i\\)'s from CMTE, TRR, and TMDDM variants for example 1.1 and 1.2}\n")
cat("\\end{table}\n")

...
unlink(list.files("results_1.1_1.2", pattern = "^coef_est_tmddm", full.names = TRUE), force = TRUE)
...
results_log <- foreach(i = 1:nrow(param_grid), .packages = c("rTensor", "MASS", "TRES"), .combine = rbind) %dopar% {
  ...
  tryCatch({
    ...
    # Save result
    output_file <- sprintf("results_1.1_1.2/coef_est_tmddm_n%d_p%d_r%s_eps%.2f_fn%d_rep%d.RData",
                           n, p, r_str, eps, f_num, n_rep)
    save(
      tmddm_acc_list,
      tmddm_time_list,

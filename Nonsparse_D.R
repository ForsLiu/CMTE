...
MMDDM <- function(Y,X){
  ...
  MMDDM_right<-matrix(0,p[2],p[2]) 
  MMDDM_left<-matrix(0,p[1],p[1]) 
  mu = apply(Y, c(1,2), mean)
  mmu = array(0, c(p[1],p[2],(n)))
  for (i in 1:(n)){
    mmu[,,i] = mu
  }
  Y = Y - mmu
  
  B_left<-matrix(0,p[1], p[1])
  B_right<-matrix(0,p[2], p[2])
  for(i in 1:(n)){
    for(j in 1:(n)){
      B_left <- B_left+Y[,,j]%*%t(Y[,,i])*vecnorm(X[,j]-X[,i])
      B_right <- B_right+t(Y[,,j])%*%(Y[,,i])*vecnorm(X[,j]-X[,i])
    }
    
  }
  MMDDM_left<- -B_left/((n)^2)
  MMDDM_right<--B_right/((n)^2)
  
  MMDDM<- list(Left = MMDDM_left, Right = MMDDM_right)
}
...
p1<-100
p2<-100
A_1<-matrix(runif(p1*u1,-1,1), p1,u1)
A_1 <- qr.Q(qr(A_1))
A_2<-matrix(runif(p2*u2,-1,1), u2, p2)
A_2 <- qr.Q(qr(t(A_2)))
#A_2<-t(A_2)
Gamma<-NULL
Gamma[[1]]<-A_1
...
DGP<-function(n,d1,d2){#,d4,d3
  #k1*k2 is the dimension of the factor series
  k1=u1
  k2=u2
  u<-c(u1, u2)
  r<-c(d1,d2)
  ...
  #k4=2
  m=2
  #inter<-array(0,c(n+31,k1,k2))#,k4,k3
  data<-array(0,c(d1,d2,n))#,d4,d3
  #epsilon1<-array(rnorm((n)*(k1*k2),0,1),c(n,k1,k2))#*k4,k4*k3,k3
  eta <- array(runif(prod(u) * k1), c(u, k1))
  eta <- rTensor::as.tensor(eta)
  B <- rTensor::ttl(eta, Gamma, ms = 1:m)
  Xn <- matrix(rnorm(k1*n),k1,n)
  Epsilon <- array(rnorm(prod(r) * n), c(r, n))
  Epsilon <- rTensor::as.tensor(Epsilon)
  Yn <- 0.1*Epsilon + rTensor::ttm(B, t(((Xn))), m + 1)
  
  
  #X<-array(0,c(k1,k2,n))
  
  
  #ttl(ttmp, lizt, 1:2)+0.1*epsilon2[j,,]
  
  DGP<-list(Y=Yn, X=(Xn))
}


args<-commandArgs(trailingOnly=TRUE) # needed when using campus cluster
isim<-as.integer(args[1])# needed when using campus cluster
...
##STORE##
#data2<-list(Yn, Xn)
result <- mystor(Y,t(Xn), Para=rep(100,u1), K=5)
Beta.hat<-result$Beta.hat

Beta.hat_1 <- qr.Q(qr(t(Beta.hat[[1]])))
D_STORE_1<-norm(A_1%*%t(A_1)- Beta.hat_1%*%t( Beta.hat_1), type="F")
#  tmp<-result$Beta.hat[[2]]
Beta.hat_2 <- qr.Q(qr(t(Beta.hat[[2]])))
D_STORE_2<-norm((A_2)%*%t(A_2)- Beta.hat_2%*%t( Beta.hat_2), type="F")

##TRR##
u<-c(u1,u2)
res_ECD <- TRR.fit(Xn, Yn, u, method="ECD")
Gamma_hat<-res_ECD$Gamma

D_TRR_1<-norm(A_1%*%t(A_1)-Gamma_hat[[1]]%*%t(Gamma_hat[[1]]), type="F")
D_TRR_2<-norm((A_2)%*%t(A_2)-Gamma_hat[[2]]%*%t(Gamma_hat[[2]]), type="F")

##TMDDM##
result <- MMDDM(Yn, Xn)
result_eg<-eigen(result$Left)
Gamma_M_Left<-result_eg$vectors[,1:u1]

result_eg<-eigen(result$Right)
Gamma_M_Right<-result_eg$vectors[,1:u2]

D_M_1<-norm(A_1%*%t(A_1)-Gamma_M_Left%*%t(Gamma_M_Left), type="F")
D_M_2<-norm((A_2)%*%t(A_2)-Gamma_M_Right%*%t(Gamma_M_Right), type="F")

setwd("/nics/c/home/clee88/TensorEnvelope/Nonsparse/Linear")
###
filename <- paste("D_STORE_1",args[1],".out",sep="")
write.table(D_STORE_1,filename, col.names=FALSE, row.names=FALSE, sep=",")
filename <- paste("D_STORE_2",args[1],".out",sep="")
write.table(D_STORE_2,filename, col.names=FALSE, row.names=FALSE, sep=",")
#filename <- paste("D_STORE_3",args[1],".out",sep="")
#write.table(D_STORE_3,filename, col.names=FALSE, row.names=FALSE, sep=",")

filename <- paste("D_M_1",args[1],".out",sep="")
write.table(D_M_1,filename, col.names=FALSE, row.names=FALSE, sep=",")
filename <- paste("D_M_2",args[1],".out",sep="")
write.table(D_M_2,filename, col.names=FALSE, row.names=FALSE, sep=",")
#filename <- paste("D_M_3",args[1],".out",sep="")
#write.table(D_M_3,filename, col.names=FALSE, row.names=FALSE, sep=",")

filename <- paste("D_TRR_1",args[1],".out",sep="")
write.table(D_TRR_1,filename, col.names=FALSE, row.names=FALSE, sep=",")
filename <- paste("D_TRR_2",args[1],".out",sep="")
write.table(D_TRR_2,filename, col.names=FALSE, row.names=FALSE, sep=",")
##filename <- paste("D_TRR_3",args[1],".out",sep="")
##write.table(D_TRR_3,filename, col.names=FALSE, row.names=FALSE, sep=",")
...
A_1<-matrix(runif(p1*u1,-1,1), p1,u1)
A_1 <- qr.Q(qr(A_1))
B_1 <- (qr.Q(qr(A_1),complete=TRUE)[,(u1+1):p1]) # p1*(p1-u1)
A_2<-matrix(runif(p2*u2,-1,1), u2, p2)
A_2 <- qr.Q(qr(t(A_2)))
B_2 <- (qr.Q(qr(A_2),complete=TRUE)[,(u2+1):p2]) # p2*(p2-u1)
#A_2<-t(A_2)
Gamma<-NULL
Gamma[[1]]<-A_1
Gamma[[2]] <- A_2
...
DGP<-function(n,d1,d2){#,d4,d3
  ...
  Epsilon <- array(rnorm(prod(r) * n), c(r, n))
  Epsilon <- rTensor::as.tensor(Epsilon)
  Yn <- 0.1*Epsilon + rTensor::ttm(B, t(exp(abs(Xn))), m + 1)
...
setwd("/nics/c/home/clee88/TensorEnvelope/Nonsparse/Nonlinear1")
###
filename <- paste("D_STORE_1",args[1],".out",sep="")
write.table(D_STORE_1,filename, col.names=FALSE, row.names=FALSE, sep=",")
filename <- paste("D_STORE_2",args[1],".out",sep="")
...
DGP<-function(n,d1,d2){#,d4,d3
  ...
  Epsilon <- array(rnorm(prod(r) * n), c(r, n))
  Epsilon <- rTensor::as.tensor(Epsilon)
  Yn <- 0.1*Epsilon + rTensor::ttm(B, t((Xn)^2), m + 1)
...
setwd("/nics/c/home/clee88/TensorEnvelope/Nonsparse/Nonlinear2")
###
filename <- paste("D_STORE_1",args[1],".out",sep="")
write.table(D_STORE_1,filename, col.names=FALSE, row.names=FALSE, sep=",")
filename <- paste("D_STORE_2",args[1],".out",sep="")
